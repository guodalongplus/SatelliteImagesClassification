...
batch_size = 28
# 四种类别：沙漠、高速公路、岛屿
classes = ['desert', 'highway', 'house', 'island']
# 类别个数
...
# 把loss、accuracy等信息上传到tensorboard可视化显示
with tf.name_scope('accuracy'):
    with tf.name_scope('correct_prediction'):
        # 结果存放在一个布尔型列表中
        correct_prediction = tf.equal(y_pred_cls, y_true_cls)  # argmax返回一维张量中最大的值所在的位置
    with tf.name_scope('accuracy'):
        # 求准确率
...
session.run(tf.global_variables_initializer())
writer = tf.compat.v1.summary.FileWriter('./log', session.graph)
